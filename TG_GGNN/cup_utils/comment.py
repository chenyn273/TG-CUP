...
from cup_utils.doc_preprocessor.doc_preprocessor import DocPreprocessor
from cup_utils.javatokenizer.tokenizer import tokenize_text_with_con
from cup_utils.edit import match_sents
...
class JavadocDescPreprocessor:
    ...
    def _filter_trivial_sents(sents: List[List[str]]):
        ...
        for sent in sents:
            ...
            temp_sent = " ".join(sp.decode_ids(sp.EncodeAsIds(sent)))
    ...
    def _preprocess_single_desc(self, javadoc):
        ...
        # todo use bpe to tokenize
        sp = spm.SentencePieceProcessor()
        sp.Load("tokenizer/code.model")
        src_sent_tokens = [sp.EncodeAsPieces(sent) for sent in src_sents]
