...
def compute_meteor(reference_list, sentences):
    ...
    return 100 * sum(meteor_scores) / len(meteor_scores), meteor_scores
...
def compute_sari(source_sentences, target_sentences, predictions):
    ...
    return 100 * sum(scores) / float(len(scores)), scores
...
def compute_accuracy(reference_strings, predicted_strings):
    ...
    return 100 * correct / float(len(reference_strings)), acc
...
def compute_metrics(src_file, ref_file, pred_file):
    ...
    predicted_accuracy, acc_list = compute_accuracy(gold_strs, pred_strs)
    # predicted_recall = compute_recall(gold_strs, pred_strs)
    predicted_meteor, meteor_list = compute_meteor(gold_tokens, pred_tokens)
    predicted_sari, sari_list = compute_sari(src_strs, gold_strs, pred_1_str)
...
if __name__ == '__main__':
    src_file = "/Users/chenyn/研/论文撰写/Updater/实验/tools/eval_tools/prediction/64-3/src.txt"
    ref_file = '/Users/chenyn/研/论文撰写/Updater/实验/tools/eval_tools/prediction/64-3/ref.txt'
    pred_file = '/Users/chenyn/研/论文撰写/Updater/实验/tools/eval_tools/prediction/64-3/pred.txt'
