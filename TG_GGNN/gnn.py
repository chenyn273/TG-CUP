...
class GatedGraphNeuralNetwork(nn.Module):
    ...
    def compute_node_representations(self,
        ...
        message_targets = torch.cat(message_targets, 0)  # Shape [M]
        ...
        for layer_idx, num_timesteps in enumerate(self.layer_timesteps):
            ...
            for t in range(num_timesteps):
                ...
                message_source_states: List[
                    torch.FloatTensor] = []  # list of tensors of edge source states of shape [E, D]
...
def main():
    gnn = GatedGraphNeuralNetwork(hidden_size=64, num_edge_types=3,
                                  layer_timesteps=[3, 5, 7, 2], residual_connections={2: [0], 3: [0, 1]},
                                  state_to_message_dropout=0.1, rnn_dropout=0.1)

    adj_list_type1 = AdjacencyList(node_num=4, adj_list=[(0, 2), (2, 1), (1, 3)], device=gnn.device)
    adj_list_type2 = AdjacencyList(node_num=4, adj_list=[], device=gnn.device)
    adj_list_type3 = AdjacencyList(node_num=4, adj_list=[], device=gnn.device)
